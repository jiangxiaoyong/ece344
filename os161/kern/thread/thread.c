...
/*
 *
 *create a new procee based on an existing one
 *
 */
...
//		newguy->t_cwd = curthread->t_cwd;
		newguy->t_cwd = kmalloc(sizeof(struct vnode));
       ...
       /* allocate page table directory for this thread or process*/
	pd = page_table_initializer();
	newguy->page_directory_addr = pd;
	...
	{
 	      /*allocate sapce*/
      		 /* newguy->t_vmspace = kmalloc(sizeof(struct addrspace));*/
		/*memcpy(newguy->t_vmspace, curthread->t_vmspace, sizeof(struct addrspace));*/
		
		if(as_copy(curthread->t_vmspace, &newguy->t_vmspace, curthread->page_directory_addr, newguy->page_directory_addr) != 0)
		{
			kfree(newguy->t_name);
			kfree(newguy->t_stack);
			kfree(newguy->t_cwd);
	...
	if( curthread->t_sleepaddr != NULL )
	{
       ...
       /* copy trapframe*/
	struct trapframe *temp_trapframe = kmalloc(sizeof(struct trapframe));
	if(temp_trapframe == NULL)
	{
			kfree(newguy->t_name);
			kfree(newguy->t_stack);
			kfree(newguy->t_cwd);
			kfree(newguy->t_vmspace);
			kfree(newguy);
			return ENOMEM;			
	}

	if(memcpy(temp_trapframe, (struct trapframe *)data1, sizeof(struct trapframe)) == NULL)
	{
		goto fail;		
	}

       /* copy pcb_copyjmp*/	
	if(memcpy(newguy->t_pcb.pcb_copyjmp,curthread->t_pcb.pcb_copyjmp, sizeof(jmp_buf)) == NULL)
	{
		goto fail;	
	}

       /* initialize pcb of new process*/
	md_initpcb(&newguy->t_pcb, newguy->t_stack, temp_trapframe , newguy->t_vmspace, func);

       /* allocate process Id for this child process*/
	result = allocatePid(&child_processID, newguy);
	if(result)
	{
		return result;	
	}
	newguy->pid = child_processID;
	*c_id = child_processID;
...
mi_switch(threadstate_t nextstate)
{
	...
	curthread = next;
       ...
       /* do TLB entries flush before context switch*/
	for( i = 0 ; i < NUM_TLB; i++  )
	{
